...
class DotNetVulnerabilities():
    ...
    def get_top_level_packages(self) -> dict:
        return self.dotnet_vulns_dicts["projects"][0]["frameworks"][0]["topLevelPackages"]

    def get_transitive_level_packages(self) -> dict:
        return self.dotnet_vulns_dicts["projects"][0]["frameworks"][0]["transitivePackages"]
    ...
    def print_vulns(self, ado: bool):
        all_vulns = list(chain(self.critical_vulns, self.high_vulns, self.moderate_vulns))

        for vuln in all_vulns:
            if ado:
                print(f"##[Error]Package {vuln['package_name']} has a {vuln['severity']} risk issue. Advisory: {vuln['advisory_url']} ({vuln['package_type']})")
            print(f"Package name: {vuln['package_name']}\nPackage type: {vuln['package_type']}\nSeverity: {vuln['severity']}\nAdvisory: {vuln['advisory_url']}\n\n")
    ...
    def failure_check(self, failure_level: str):
        if failure_level == "critical":
            if len(self.critical_vulns) > 0:
                ...
                print("Failure threshold not met.")
            ...
            if len(list(chain(self.critical_vulns, self.high_vulns, self.moderate_vulns))) > 0:
                exit(1)
            else:
                print("Failure threshold not met.")


def categorise_package_vulnerabilities(packages: list, dotnet_vulnerabilities: DotNetVulnerabilities, package_type: str):
...
def check_dotnet_deps(include_transitive: bool, path: str) -> DotNetVulnerabilities:
    dotnet_cmd = subprocess.run(["dotnet", "list", path, "package", "--vulnerable", "--include-transitive", "--format", "json"], capture_output=True, encoding="utf-8")
    ...
    categorise_package_vulnerabilities(top_level_packages, dotnet_vulnerabilities, package_type="top_level")

    if include_transitive:
        transitive_packages = dotnet_vulnerabilities.get_transitive_level_packages()
        categorise_package_vulnerabilities(transitive_packages, dotnet_vulnerabilities, package_type="transitive")
...
def main():
    ...
    if dotnet_check:
        ...
        if args.failure_level in failure_threshold_options:
            ...
            print("Valid failure level not provided.")
    else:
